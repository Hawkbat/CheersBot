...
import { WinLossData, ControlPanelViewData, ControlPanelPage } from 'shared'
import { PanelField } from '../controls/PanelField'
import { channelAction } from '../utils'
import { Button } from '../controls/Button'
import { Toggle } from 'src/controls/Toggle'

export function WinLossPanel(props: ControlPanelViewData & WinLossData & { page: ControlPanelPage }) {
        ...
        case ControlPanelPage.view:
            return <>
                <PanelField label="Display">
                    <Toggle value={props.display} onToggle={v => setDisplayed(v)} />
                </PanelField>
                <PanelField label="Wins ($win)">
                    <input type="number" disabled value={props.wins} />&nbsp;
                    <Button onClick={e => adjustWins(+1)}>+1</Button>&nbsp;
                    <Button onClick={e => adjustWins(-1)}>-1</Button>
                </PanelField>
                <PanelField label="Losses ($loss)">
                    <input type="number" disabled value={props.losses} />&nbsp;
                    <Button onClick={e => adjustLosses(+1)}>+1</Button>&nbsp;
                    <Button onClick={e => adjustLosses(-1)}>-1</Button>
                </PanelField>
                <PanelField label="Draws ($draw)">
                    <input type="number" disabled value={props.draws} />&nbsp;
                    <Button onClick={e => adjustDraws(+1)}>+1</Button>&nbsp;
                    <Button onClick={e => adjustDraws(-1)}>-1</Button>
                </PanelField>
                <PanelField label="Deaths ($death)">
                    <input type="number" disabled value={props.deaths} />&nbsp;
